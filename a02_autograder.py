    ...
    for tc in test_cases:
        ...
        try:
            ...
            if exit_code != 0:
                ...
                # For validation tests, check if the program printed
                # a real error message before crashing (e.g., via
                # sys.exit() after print). Only count it if the output
                # contains the expected validation pattern — not just
                # leftover prompts from input() calls.
                if tc_type == "validation" and stdout:
                    if re.search(expected, stdout, re.DOTALL | re.IGNORECASE):
                        # Student printed a proper error, then exited
                        output = stdout
                        crashed = False
                    else:
                        # Program crashed without a proper error message.
                        # Common cause: using int()/float() on raw input
                        # without checking first.
                        if "ValueError" in last_line:
                            notes.append(
                                f"  CRASH [{label}]: Program crashed on "
                                f"invalid input instead of showing an "
                                f"error message."
                            )
                            notes.append(
                                f"    Tip: Validate the input BEFORE "
                                f"converting with int() or float()."
                            )
                            notes.append(
                                f"    See the .isdigit() or .replace() "
                                f"techniques from the reading."
                            )
                        else:
                            notes.append(
                                f"  ERROR [{label}]: Program crashed"
                            )
                            if "EOFError" in last_line:
                                notes.append(
                                    f"    Program asked for more input "
                                    f"than expected."
                                )
                            else:
